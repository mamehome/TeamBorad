...
function defaultColor(type){
  const map = {
    attack:"#e53935", defense:"#1e88e5", free:"#fdd835", gk:"#9e9e9e", person: currentPartColor || "#ffffff", cone:"#ff7a00", marker:"#ffea00", ladder:"#ffb74d",
...
function isFixedColorType(type){ return ["attack","defense","free","gk","cone","marker","ladder"].includes(type); }
function isColorableType(type){ return ["person","circlePart","squarePart","hLine","vLine","centerLine","centerCircle","courtArea","goalFrame","line","arrow","dash","text"].includes(type); }
function objColor(o){ return isFixedColorType(o ? o.type : activeTool) ? defaultColor(o ? o.type : activeTool) : (o && o.color ? o.color : defaultColor(o ? o.type : activeTool)); }
...
function ladderShape(x,y,r=0,s=1,color="#ffb74d"){
  const w = 56*s, h = 118*s;
  const sw = Math.max(2.5, 5*s);
  const rung = Math.max(2, 4*s);
  const left = x - w/2, right = x + w/2, top = y - h/2, bottom = y + h/2;
  const rungs = [0.18,0.34,0.50,0.66,0.82].map(t=>{
    const yy = top + h*t;
    return `<line x1="${left}" y1="${yy}" x2="${right}" y2="${yy}" stroke="${color}" stroke-width="${rung}" stroke-linecap="round"/>`;
  }).join("");
  return `<g transform="rotate(${r} ${x} ${y})">
    <line x1="${left}" y1="${top}" x2="${left}" y2="${bottom}" stroke="${color}" stroke-width="${sw}" stroke-linecap="round"/>
    <line x1="${right}" y1="${top}" x2="${right}" y2="${bottom}" stroke="${color}" stroke-width="${sw}" stroke-linecap="round"/>
    ${rungs}
  </g>`;
}
function arrowLineShape(o, dashed=false){
  const c = objColor(o);
  const s = o.s || 1;
  const sw = Math.max(4, (dashed ? 7 : 8) * s);
  const dx = (o.x2 || 0) - (o.x1 || 0);
  const dy = (o.y2 || 0) - (o.y1 || 0);
  const len = Math.hypot(dx, dy) || 1;
  const ux = dx / len, uy = dy / len;
  const size = Math.max(18, 28*s);
  const bx = o.x2 - ux * size;
  const by = o.y2 - uy * size;
  const px = -uy, py = ux;
  const p1x = bx + px * size * .42, p1y = by + py * size * .42;
  const p2x = bx - px * size * .42, p2y = by - py * size * .42;
  const lineEndX = o.x2 - ux * size * .34;
  const lineEndY = o.y2 - uy * size * .34;
  const dashAttr = dashed ? ` stroke-dasharray="${18*s} ${16*s}"` : "";
  return `<line x1="${o.x1}" y1="${o.y1}" x2="${lineEndX}" y2="${lineEndY}" stroke="${c}" stroke-width="${sw}" stroke-linecap="round"${dashAttr}/>
    <polygon points="${o.x2},${o.y2} ${p1x},${p1y} ${p2x},${p2y}" fill="${c}" stroke="${c}" stroke-width="${Math.max(1,2*s)}" stroke-linejoin="round"/>`;
}
...
function shapeSvg(o){
  ...
  const s = o.s || 1;
  const c = objColor(o);
  if(o.type==="attack") return playerShape(o.x,o.y,"#e53935","A","#fff",s,o.r||0);
  if(o.type==="defense") return playerShape(o.x,o.y,"#1e88e5","D","#fff",s,o.r||0);
  if(o.type==="free") return playerShape(o.x,o.y,"#fdd835","F","#17211b",s,o.r||0);
  if(o.type==="gk") return playerShape(o.x,o.y,"#9e9e9e","GK","#fff",s,o.r||0);
  if(o.type==="person") return playerShape(o.x,o.y,c || "#ffffff","人", c && c.toLowerCase()==="#fdd835" ? "#17211b" : "#fff",s,o.r||0);
  if(o.type==="cone") return coneShape(o.x,o.y,s);
  if(o.type==="marker") return markerShape(o.x,o.y,s,"#ffea00");
  if(o.type==="ladder") return ladderShape(o.x,o.y,o.r||0,s,"#ffb74d");
  ...
  if(o.type==="arrow") return arrowLineShape(o,false);
  if(o.type==="dash") return arrowLineShape(o,true);
  return "";
}
...
function objectBounds(o){
  ...
  const boxes = {
    attack:[56*s,56*s], defense:[56*s,56*s], free:[56*s,56*s], gk:[64*s,56*s], person:[60*s,56*s], cone:[58*s,58*s], marker:[40*s,40*s], ladder:[78*s,140*s],
...
function previewDrawingSvg(){
  ...
  if(type==="arrow") return arrowLineShape(o,false);
  if(type==="dash") return arrowLineShape(o,true);
  return "";
}
...
function toolLabelName(t){
  ...
  const names = {
    attack:"攻撃", defense:"守備", free:"フリー", gk:"GK", person:"人", cone:"コーン", marker:"マーカー", ladder:"ラダー",
...
function syncPartColorInput(){
  ...
  input.disabled = !!(o && isFixedColorType(o.type));
}
...
function createObjectAt(p){
  ...
  if(activeTool==="text"){
    ...
    obj = {id:uid(), type:activeTool, x:p.x, y:p.y, r:currentRotation, s:1, color:isFixedColorType(activeTool) ? defaultColor(activeTool) : currentPartColor};
  }
